...
Se han tenido en cuenta las dos herramientas más usadas en proyectos con Go:
   ...
   **Estandar de Go** (testing): https://pkg.go.dev/testing
   Instalada de forma nativa junto con el lenguaje, el cual utiliza "testing" comp paquete, el cual, tecnicamente no es una bilbioteca de aserciones, pero puede realizar las mismas acciones que esta de otra forma, obteniendo el mismo resultado.

   **Testify**: https://github.com/stretchr/testify 
   Requiere de su instalación adicional ya que no está integrado en Go.

   **assert**: https://github.com/alecthomas/assert 
   Biblioteca de aserciones basada en Testify, pero más reducida. También requiere de su instalación adicional ya que no está integrado en Go.

   **Ghost**: https://github.com/rliebz/ghost 
   Biblioteca de aserciones menos conocida, muy flexible. También requiere de su instalación adicional ya que no está integrado en Go.
...
Cabe destacar, solo como apunte extra sin entrar en los criterior tomados, que mientras muchas bibliotecas hacen uso de assert(), testing, al no ser exactamente una biblioteca de aserciones, promueve el uso de if() y t.Errorf para reportar errores en los tests, lo cual permite el lanzamiento de errores más personalizados y descriptivos para cada ocasión.
...
Se usará la propia del lenguaje ya que la mejor práctica es usar la que ya hay. Se ha seleccionado Go Test no solo como herramienta CLI, sino como Test Runner, como se ha indicado anteriormente, ejecutando esta herramienta ambas funciones, manteniendo la consistencia durante todo el test.

En caso de haber elegido ginkgo, podríamos haber utilizado su herramienta CLI propia pues este también la incluye integrada. 

En resumen, Go Test es una herramienta integral capaz de actuar en los tres niveles de herramientas, gestionando la ejecución de tests, generando los reportes de errores, y sustituye la función de biblioteca de aserciones, por el paquete testing, que pese a no ser una biblioteca de aserciones, permite realizar las tareas en código que esta haría.
